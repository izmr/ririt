...
class RiritInfo
  ...
  def self.get_shop_item_hash(text)
    ...
    if(!urls.empty?)
      urls.each do |url|
        ...
        if (url)
          $log.info('[RiritInfo.get_shop_item_hash] URL: ' + url.to_s)
          ...
          $log.info('[RiritInfo.get_shop_item_hash] shop_item_code: ' + uri.join(':'))
          @@rakuten_url = url
  ...
  def self.expand(url, prev_location="", count=0)
    $log.info('[RiritInfo.expand] URI:' + url.to_s + ', COUNT:' + count.to_s)
    return nil if(count > 5)
    return nil if (url.to_s.match(/^http:\/\/twitpic\.com/))
    return url if (url.to_s.match(/^http:\/\/ririt\.marimofire\.com/))
      ...
      http.start do |http|
        ...
        if(locations)
          locations.each do |location|
            if (location.match(/^http:\/\/ririt\.marimofire\.com/))
